...
| 上游内容 | 必需性 | 用途 | 缺失时怎么做 |
|----------|--------|------|--------------|
| `全局资源` 页面及其子页 | 必需 | 判断模块对你们项目的推荐级别和适用场景 | 如果没有，仍可写报告，但要明确推荐级别可信度下降 |
...
5. 在对应多维表内按 `模块名` 查找记录；有则更新，没有则新建
...
- `模块名` 使用标题列
- `简介` 使用文本字段
- `推荐级别` 使用 `select` 单选字段，选项固定为 `推荐 / 备选 / 谨慎`
- `适用场景` 使用文本字段
