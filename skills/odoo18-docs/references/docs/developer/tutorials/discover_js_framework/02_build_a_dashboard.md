...
source_url: "https://docs.odoo.sbggai.top/developer/tutorials/discover_js_framework/02_build_a_dashboard.html"
---
...
]({GITHUB_PATH}/addons/web/static/src/search/layout.js), available in `@web/search/layout`.
...
.. seealso::
   ...
   - [Example: use of Layout in client action
     ]({GITHUB_PATH}/addons/web/static/src/webclient/actions/reports/report_action.js) and
   ...
   - [Example: use of Layout in kanban view
     ]({GITHUB_PATH}/addons/web/static/src/views/kanban/kanban_controller.xml)
...
#. A button `Leads`, which opens a dynamic action on the `crm.lead` model with a list and a form
   ...
   /static/src/components/journal_dashboard_activity/journal_dashboard_activity.js#L28-L35).
...
.. seealso::
   [Code: action service
   ]({GITHUB_PATH}/addons/web/static/src/webclient/actions/action_service.js)
...
.. seealso::
   [Owl's slot system ]({OWL_PATH}/doc/reference/slots.md)
...
.. seealso::
   [Code: rpc service ]({GITHUB_PATH}/addons/web/static/src/core/network/rpc_service.js)
...
#. Use the [memoize ](https://github.com/odoo/odoo/blob/1f4e583ba20a01f4c44b0a4ada42c4d3bb074273/
   addons/web/static/src/core/utils/functions.js#L11) utility function from
   `@web/core/utils/functions` that allows caching the statistics.
#. Use this service in the `Dashboard` component.
#. Check that it works as expected.

.. seealso::
   - [Example: simple service ]({GITHUB_PATH}/addons/web/static/src/core/network/http_service.js)
...
#. Use the `PieChart` component in a `DashboardItem` to display a [pie chart
   ](https://www.chartjs.org/docs/2.8.0/charts/doughnut.html) that shows the
...
#. Modify it to return a [reactive ]({OWL_PATH}/doc/reference/reactivity.md#reactive) object.
   Reloading data should update the reactive object in place.
#. The `Dashboard` component can now use it with a `useState`

.. seealso::
  - [Documentation on reactivity ]({OWL_PATH}/doc/reference/reactivity.md)
