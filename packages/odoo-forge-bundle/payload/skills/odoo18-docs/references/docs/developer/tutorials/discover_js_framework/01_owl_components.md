---
title: "Chapter 1: Owl components"
source_url: "https://docs.odoo.sbggai.top/developer/tutorials/discover_js_framework/01_owl_components.html"
---
...
]({OWL_PATH}/doc/reference/component.md) and [templates ]({OWL_PATH}/doc/reference/templates.md).
...
.. spoiler:: Solutions
   ...
   repository
   ](https://github.com/odoo/tutorials/commits/{CURRENT_MAJOR_BRANCH}-discover-js-framework-solutions/awesome_owl). It
...
#. Modify :file:`playground.js` so that it acts as a counter like in the example above.
   ...
   ]({OWL_PATH}/doc/reference/hooks.md#usestate) so that the component is re-rendered
...
#. Modify the template in :file:`playground.xml` so that it displays your counter variable. Use
   [t-esc ]({OWL_PATH}/doc/reference/templates.md#outputting-data) to output the data.
#. Add a button in the template and specify a [t-on-click
   ]({OWL_PATH}/doc/reference/event_handling.md#event-handling) attribute in the button to
...
This exercise showcases an important feature of Owl: the [reactivity system ]({OWL_PATH}/doc/reference/reactivity.md).
...
see how to create a [sub-component ]({OWL_PATH}/doc/reference/component.md#sub-components) from it:
...
sub component by using attributes (most commonly known as [props ]({OWL_PATH}/doc/reference/props.md)).
...
[t-out directive ]({OWL_PATH}/doc/reference/templates.md#outputting-data).
...
]({OWL_PATH}/doc/reference/app.md#dev-mode). You can activate the dev mode in the [App
configuration ]({OWL_PATH}/doc/reference/app.md#configuration) (but it is activated by default
on the `awesome_owl` playground).

It is a good practice to do props validation for every component.

#. Add [props validation ]({OWL_PATH}/doc/reference/props.md#props-validation) to the `Card`
   component.
...
This can be done by using a [callback prop ]({OWL_PATH}/doc/reference/props.md#binding-function-props):
...
>    There is a subtlety with callback props: they usually should be defined with the `.bind`
   suffix. See the [documentation ]({OWL_PATH}/doc/reference/props.md#binding-function-props).
...
#. Use [t-foreach ]({OWL_PATH}/doc/reference/templates.md#loops) to display each todo in a `TodoItem`.
...
using a [dynamic attributes ]({OWL_PATH}/doc/reference/templates.md#dynamic-attributes).
...
>    Owl let you combine static class values with dynamic values. The following example will work as expected:
   ...
   See also: [Owl: Dynamic class attributes ]({OWL_PATH}/doc/reference/templates.md#dynamic-class-attribute)
...
#. Add an [event handler ]({OWL_PATH}/doc/reference/event_handling.md) on the `keyup` event
   named `addTodo`.
...
So far, we have seen one example of a hook function: `useState`. A [hook ]({OWL_PATH}/doc/reference/hooks.md)
...
mounted, updated, detached, destroyed... This is the [component lifecycle ]({OWL_PATH}/doc/reference/component.md#lifecycle).
...
Owl provides a variety of built-in [hooks functions ]({OWL_PATH}/doc/reference/hooks.md). All of them have to be called in
...
Let's see how we can access the DOM with [t-ref ]({OWL_PATH}/doc/reference/refs.md) and [useRef
]({OWL_PATH}/doc/reference/hooks.md#useref). The main idea is that you need to mark
...
Then you can access it in the JS with the [useRef hook ]({OWL_PATH}/doc/reference/hooks.md#useref).
...
#. Bonus point: extract the code into a specialized [hook ]({OWL_PATH}/doc/reference/hooks.md)
...
]({OWL_PATH}/doc/reference/props.md#binding-function-props) `toggleState`.
...
This is exactly what Owl's [slot ]({OWL_PATH}/doc/reference/slots.md) system is designed
